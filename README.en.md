...
Install this project as an unpacked extension. These instructions do not use a browser extension store.

1. On this GitHub repository's home page, click the green **Code** button, then **Download ZIP**.
...
Do not move or delete the installed folder: Edge continues to load the extension from it. It is normal for the source ZIP to have no `dist` folder. You can load the source directory directly, without building anything.
